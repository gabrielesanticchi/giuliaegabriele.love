...
integration(
  ...
  () => {
    ...
    async function createGift(priceEuros = 10_000): Promise<string> {
      const id = randomUUID();
      giftIds.push(id);
    ...
    it("serializes concurrent contributions so their sum never exceeds the price", async () => {
      ...
      const attempts = [6_000, 6_000].map((amountEuros, index) =>
        contributeToGift(db, {
          giftId,
          amountEuros,
          idempotencyKey: randomUUID(),
    ...
    it("verifies idempotently while separating received and applied amounts", async () => {
      ...
      const verified = await verifyIntent(db, {
        intentId: second.id,
        receivedAmountEuros: 4_000
      });
      const repeated = await verifyIntent(db, {
        intentId: second.id,
        receivedAmountEuros: 9_000
      });

      expect(firstVerified.receivedAmountEuros).toBe(10_000);
      expect(firstVerified.appliedAmountEuros).toBe(6_000);
      expect(verified.receivedAmountEuros).toBe(4_000);
      expect(verified.appliedAmountEuros).toBe(4_000);
      expect(repeated.receivedAmountEuros).toBe(4_000);
      expect(repeated.appliedAmountEuros).toBe(4_000);
    ...
    it.each([
      ...
      async (_name, mutation, amountEuros) => {
        const giftId = await createGift();
        const idempotencyKey = randomUUID();
        const input = {
          giftId,
